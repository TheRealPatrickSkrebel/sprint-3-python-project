...
# a) Games with global sales greater than 25 million
for game in video_game_sales:
    if game[GLOBAL_SALES] > 25:
        print(f"{game[NAME]}: {game[GLOBAL_SALES]}")
...
print(f"Pre-2000 games count: {pre_2000_count}")

# c) Total NA sales vs. total JP sales and higher sales region comparison
total_na_sales = 0
total_jp_sales = 0
...
print(f"Total NA Sales: {round(total_na_sales, 2)}")
print(f"Total JP Sales: {round(total_jp_sales, 2)}")
...
# d) List of games published by Nintendo
nintendo_games = []
for game in video_game_sales:
