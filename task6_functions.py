...
# a) Function to calculate combined NA, EU, and JP sales for a single game
def calculate_total_sales(game):
    return game[NA_SALES] + game[EU_SALES] + game[JP_SALES]

# Test on the first game in the dataset (Wii Sports)
first_game_regional_sales = calculate_total_sales(video_game_sales[0])
print(first_game_regional_sales)


# b) Function to filter games by genre with a default value of 'Platform'
...
# Test with default genre ('Platform')
platform_games = filter_by_genre(video_game_sales)
print(f"Platform games count: {len(platform_games)}")

# Test with specified genre ('Sports')
sports_games = filter_by_genre(video_game_sales, 'Sports')
print(f"Sports games count: {len(sports_games)}")


# c) Function to get a formatted summary string of a game
def get_summary(game):
    return f"{game[NAME]} ({game[YEAR]}) - {game[GENRE]} - ${game[GLOBAL_SALES]}M"

# Loop to print a summary for every game in the dataset
